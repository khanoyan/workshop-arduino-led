...
int getPot(int pot_num) {
  ...
  return(map(pot_val, 0, 1023, 0, 255)); // makes pot value compatible with RGB values
}
...
void lightShow2(int pot1_val, int pot2_val) { 
  ...
  strip.setPixelColor(random (pixel_pos), r, g, b); //for a random color on a random pixel
  strip.show();
  delay(100);
}
